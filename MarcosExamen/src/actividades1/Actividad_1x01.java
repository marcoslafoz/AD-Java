	...
	public static void main(String[] args) {
		...
		while (opcion != 0) {
			...
			case 5:
				...
				int codigoJugadorEliminar = Teclado.leerEntero("Introduce el codigo del jugador a eliminar: ");
				Jugador jugadorEliminar = AccesoJugador.consultarJugadorPorCodigo(codigoJugadorEliminar);
				Equipo equipoJugadorEliminar = AccesoJugador.consultarEquipoDeJugador(jugadorEliminar);

				if (equipoJugadorEliminar != null) {
					System.out.println("Error al eliminar el jugador " + equipoJugadorEliminar.toString());

				} else {
					System.out.println("El jugador no pertenece a ningun equipo" + jugadorEliminar.toString());
					jugadorEliminado = AccesoJugador.eliminarJugador(codigoJugadorEliminar);
